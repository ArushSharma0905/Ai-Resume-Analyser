...
export default function DashboardHome({
  ...
  // Calculate highest match score from session
  const bestMatch = sessionMatches.length > 0
    ? [...sessionMatches].sort((a, b) => b.matchResult.overallScore - a.matchResult.overallScore)[0]
    : null;
  ...
  return (
    <div className="space-y-8 py-4 sm:py-6">
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          <div className="space-y-2 max-w-xl">
            ...
            <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight">
              {hasResume
                ? `Welcome back, ${parsed?.contact?.name || "Candidate"}!`
                : "Welcome to ResumeAI Career Suite"}
            </h1>
            <p className="text-xs sm:text-sm text-blue-100 leading-relaxed">
              {hasResume
                ? `Your profile is active for ${parsed?.targetRoleOrTitle || "your target role"} with ${parsed?.totalYearsExperience || 0} years estimated experience.`
      ...
      {/* ================================================================ */}
      {/* 2. CORE STATUS METRICS GRID                                      */}
      {/* ================================================================ */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        {/* Metric 1: Resume Status */}
        ...
        <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-5 shadow-xs flex flex-col justify-between space-y-4">
          ...
          >
            <span>{atsReport ? "Review Score Breakdown" : "Run ATS Diagnostic"}</span>
            <ChevronRight className="w-3.5 h-3.5" />
          </button>
        </div>
        ...
        <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-5 shadow-xs flex flex-col justify-between space-y-4">
          ...
          {bestMatch ? (
            <div>
              ...
              <p className="text-[11px] text-zinc-500 truncate">{bestMatch.job.company}</p>
            </div>
          ) : (
            <div>
              <p className="text-sm font-bold text-zinc-800 dark:text-zinc-200">
          ...
          <button
            type="button"
            onClick={() => onSelectTab(sessionMatches.length > 0 ? "matches" : "jobs")}
          ...
          >
            <span>{sessionMatches.length > 0 ? `View ${sessionMatches.length} Matches` : "Search Jobs to Match"}</span>
            <ChevronRight className="w-3.5 h-3.5" />
          </button>
        </div>

        {/* Metric 4: Optimization Hub */}
        <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-5 shadow-xs flex flex-col justify-between space-y-4">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">
              Resume Optimizer
          ...
          >
            <span>{optimizationResult ? "Open Optimizer" : "Explore Optimizer"}</span>
            <ChevronRight className="w-3.5 h-3.5" />
          </button>
        </div>
      </div>
      ...
      {/* ================================================================ */}
      {/* 4. RECENT ANALYZED MATCHES SECTION                               */}
      {/* ================================================================ */}
      <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-3xl p-6 sm:p-8 space-y-4 shadow-xs">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          <div>
            ...
            <p className="text-xs text-zinc-500 dark:text-zinc-400">
              Jobs you evaluated for resume compatibility during this session.
            </p>
          </div>
                ...
                >
                  <div className="space-y-1">
                    ...
                    <p className="text-xs text-zinc-500">
                      {item.job.company} • {item.job.location} • {item.matchResult.recommendation}
                    </p>
                  </div>
